...
use snforge_std::{declare, ContractClassTrait, DeclareResultTrait, 
    spy_events, start_cheat_caller_address, stop_cheat_caller_address, EventSpyAssertionsTrait, set_balance, Token};
...
fn test_contract_initialization(){
    let dispatcher: ICounterDispatcher = deploy_counter(5);
...
fn test_increase_counter(){
    ...
    assert!(current_counter == 1, "Increase counter function doesnt work");
...
fn test_decrease_counter_happy_path(){
    ...
    assert!(current_counter == 3, "Decrease counter function doesnt work");
...
fn test_decrease_counter_unhappy_path(){
    let init_counter: u32 = 0;
    let dispatcher: ICounterDispatcher = deploy_counter(init_counter);
...
fn test_reset_counter_success(){
    ...
    assert!(counter.get_counter()==0, "Unable to reset the counter even with enough STRK");
    ...
    assert!(erc20.balance_of(user)==strk_to_fri(9), "Balance not deducted correctly");
    assert!(erc20.balance_of(owner_address())==strk_to_fri(1), "Balance not increased correctly");
}